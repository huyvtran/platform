<?php
$email = $this->request->query['email_contact'];
$count  =   strlen($email);
...
<div class="box-thongtin">
    ...
    <p class="rs tt-text">
        <?php echo __("Hãy nhập mã xác thực gửi vào email <strong>%s</strong> để hoàn tất . Nếu chưa nhận được email xin vui lòng chờ ít phút hoặc kiểm tra mục Thư rác/Spam/Junk.",$email) ;?>
    </p>
    <div class="box-ttBtn box-ttForm cf">
        ...
        ?>
            ...
            <p class="rs tt-textLast"><?php echo __("Không nhận được mã xác thực?") ;?>
                <a href="<?php echo $this->Html->url(array('controller'=>'UserInfors','action' => 'sendAgainCodeEmailSecurity','?'=>array_merge(array('email_contact'=> $this->request->query['email_contact']),$ispop))) ?>"><?php echo __("Gửi lại") ;?></a></p>
