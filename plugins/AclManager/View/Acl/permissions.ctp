...
foreach ($acos as $id => $aco) {
	...
	<?php foreach ($aros as $aro): 
		...
		<td><?php
			echo $icon . " " . $this->Form->select("Perms." . str_replace("/", ":", $action) . ".{$aroAlias}:{$aro[$aroAlias]['id']}", array(array('inherit' => __('Inherit'), 'allow' => __('Allow'), 'deny' => __('Deny'))),
			array('empty' => __('No change'), 'value' => $value, 'class' => 'option-perms'));
			?>
		</td>
	<?php endforeach; ?>
<?php 
...
<script type="text/javascript">
	$(function(){
		$(".option-perms").change(function() {
			if ($(this).val() != '') {
				var data = {};
				data[this.name] = this.value;
				$.post(BASE_URL + 'acl_manager/acl/permissions', data, function(result) {
