 ...
 */
	function publish(Model $Model, $id = null, $attributes = array()) {
 ...
 * @return boolean Result of the operation.
 */
	function unPublish(Model $Model, $id = null, $attributes = array()) {
		if ($Model->hasField($this->__settings[$Model->alias]['field'])) {
			if (empty($id)) {
				$id = $Model->id;
			}
 ...
 */
	function enablePublishable(Model $Model, $methods, $enable = true) {
		if (is_bool($methods)) {
			$enable = $methods;
			$methods = array('find');
		}
 ...
 */
	function beforeFind(Model $Model, $queryData, $recursive = null) {
			...
			if ($include) {
				...
				if (is_string($queryData['conditions'])) {
					...
					if (!empty($includeDateCondition)) {
						$queryData['conditions'][$Model->alias . '.' . $this->__settings[$Model->alias]['field_date'] . ' <='] = date('Y-m-d H:i:s');
					}
				}
			}
