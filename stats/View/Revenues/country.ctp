...
<div class="box">
    <div class="box-body">
			<?php
			echo $this->Form->create('LogPaymentsCountryByDay', array('inputDefaults' => array('div' => false, 'label' => false), 'class' => 'form-inline'));
...
<table class='table table-striped table-bordered table-data responsive'>
	...
	<tbody>
		...
		foreach($data as $v) {
			...
			$class = '';
			$rate = '';
			if (isset($total_old_rev) && !empty($total_old_rev)) {
				foreach ($total_old_rev as $value) {
